...
class parallelTumoralGrowth implements Runnable{
...
//  public static JComboBox<String> CFrontera;
  ...
  public int FilasInf,FilasSup,h;

  public parallelTumoralGrowth(int FilasInf, int FilasSup,int gen){
    this.FilasInf=FilasInf;
    this.FilasSup=FilasSup;
    h=gen;
  }
...
public  void compute(){//esto sera el metodo run
  Random rd=new Random();
for(int i=FilasInf;i<FilasSup;++i){
  ...
  int g=0;
  for(int i=FilasInf;i<FilasSup;++i){//0 y n
    for(int j=0;j<nCells;++j){
      if(celulas[i][j][p]==1){//si esta muerta no podemos hacer nada
        ...
        if(rr<Ps){//si rr en menor que la probabilidad de supervivencia sobrevive
          ...
          if(proliferacion){
            celulas[i][j][q]=1;
          if(x<0 || nCells<=x || y<0 || nCells<=y){/** no ocurre nada*/}
            else{
        ...
        if(y==0){
          ...
          if(y==nCells-1){
            ...
            if(denominador!=0){
              ...
              P3=0.25;
              P4=0.25;
            //  System.out.println("Entro en 0.25");
            }
          }
                }
        }
 ...
 // 	 System.out.println("altura: "+altura+" anchura: "+ancho);
    ...
    Y2=(((double)poblacion[g]/((double)Curva.getWidth()*(double)altura))*(double)altura);
    Y2=(double)altura-Y2;
...
Start.addActionListener(new ActionListener(){
    ...
    //  Cuadricular();
      ...
      for(int j=0;j<nTareas;++j){
          if(lsup>nCells)lsup=nCells;
          pool.execute(new parallelTumoralGrowth(linf,lsup,i));
          linf=lsup;
          lsup+=tVentana;
        }
...
public static void Cuadricular(){
  ...
  int altura=Curva.getHeight();
  int incrementoAlt=(nCells*nCells)/Generaciones;
  int incrementoAnc=anchura/Generaciones;

  Curva2D.setColor(Color.BLACK);
  for(int i=0;i<altura;i+=incrementoAlt){
    Curva2D.drawLine(0,i,anchura,i);
  }
  for(int j=0;j<anchura;j+=incrementoAnc){
