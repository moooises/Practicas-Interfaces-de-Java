...
class parallelBelzab implements Runnable{
  ...
  //  public static JComboBox<String> CFrontera;
    ...
    public parallelBelzab(int FilasInf, int FilasSup){//linf,lsup,j
  ...
  public void compute(){//esto sera el metodo run
      for(int x=FilasInf;x<FilasSup;x++){
        for(int y=0;y<height;y++){
          float c_a=0.0f;
          float c_b=0.0f;
          float c_c=0.0f;
          ...
          //ystem.out.println("a: "+a[x][y][q]+" b: "+b[x][y][q]+" c: "+c[x][y][q]);
  ...
  Start.addActionListener(new ActionListener(){
      public void actionPerformed(ActionEvent e){
        ...
        for(int i=0;i<Generaciones;++i){
          ...
          if(lsup>width)lsup=width;
            pool.execute(new parallelBelzab(linf,lsup));
            linf=tVentana;
            lsup+=tVentana;
          }
          pool.shutdown();
